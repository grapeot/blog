...
Wemo and IFTTT's Maker Channel can do this job pretty well.
The only concern is, the WiFi plugin is sometimes even more expensive than the purifier itself.

In summary, the take-away message I wish you to remember is not (only) how to wire the sensors, how to write GPIO code to read the metrics, and how to write Tornado apps to host them on a website, but more importantly, the idea of test-driven development, the importance of reading data sheets, and the general idea of putting everything together to build a reliable and easy-to-use system.
