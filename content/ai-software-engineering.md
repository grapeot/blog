...
但相比之下，AI生成的个性化软件则完全是另一个套路。类似宜家家具，用户买到的不是一个开箱即用的椅子，而是一个套件。套件里有几个关键的核心部件，比如在特定的位置事先开好孔的椅面和椅腿。这些部件我们在家没办法自己造出来，而它们的质量决定了最终成品的品质。还有一张详尽的说明书，指导用户甚至AI这个全自动机器人一步步组装。用户可以按照说明书按部就班装出来一个椅子，也可以选择在这个基础上改装，换个颜色，把腿锯短一点，甚至只用三条腿，来满足自己的个性化需求。
...
1. 提供精细原始的反馈
    ...
    传统API在捕获底层错误后，一般会抛出一个抽象的高层异常。这对于AI的Agentic工作流是致命的。AI的有效性依赖于一个[尝试-反馈-修正的循环](/agentic-ai-crisis.html)。不论是对人还是对AI，模糊的“操作失败，请稍后再试”的错误信息会直接中断这个循环，让人无所适从。
