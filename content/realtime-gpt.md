...
如果说这是一个意料之中的体验提升的话，另外一个意料之外的好处是，它的语音识别质量远好于之前我用的同样是 OpenAI 自家的 [Whisper或者第三方比如AssemblyAI的模型](https://yage.ai/assemblyaiyu-yin-shi-bie-apishi-yong-gan-shou.html)。一方面，它识别明显更准，另外一方面，它对于中英文混合的情况支持得也非常好。我的理解是，这是因为 Whisper 毕竟不是一个 LLM，它的 Language Model 虽然有，但远远没有 GPT 那么大。所以虽然我们把 GPT 拿来做语音识别有点大材小用，但它还是可以很轻松地打败目前主流专门做语音识别的相对小一点的模型。
...
一个非常直观的解决方法就是我们把它从一个网页变成一个app。那这样它就可以在系统后台挂载了，每次不用重新从网页上下载，不用重新初始化麦克风，甚至网络连接都可以挂起，到前台的时候直接重连就好。稍微想一下，这个响应时间可以做得特别快。所以我就又在[Cursor的帮助](https://yage.ai/ai-coding.html)下做了一个iOS的客户端，在上面说的缓存各种资源以外，这个App还干了几件额外的事情。
